...
;typedef GB_THREAD_PROC(gbThread_Proc);
...
GB_DEF void gb_thread_start(gbThread *t, gbThread_Proc *proc, void *data);
GB_DEF void gb_thread_start_with_stack(gbThread *t, gbThread_Proc *proc, void *data, isize stack_size);
...
GB_DEF char const *gb_strchr(char const *s1, char c);
...
GB_DEF char16 *gb_from_utf8_to_utf16(char16 *buffer, char *str, isize len);
GB_DEF char   *gb_from_utf16_to_utf8(char *buffer, char16 *str, isize len);
...
typedef gbBuffer(u8) gbByte_Buffer;
...
typedef gbArray(void) gbVoid_Array; // NOTE(bill): Useful for generic stuff
...
GB_DEF void gbpriv_array_set_capacity(void *array, isize capacity, isize element_size);
...
#define gb_array_appendv(x, items, item_count) do {                                   \
	...
	gb_memcpy((x)->data[(x)->count], (items), gb_size_of((x)->data[0])*(item_count)); \
	(x)->count += (item_count);                                                       \
} while (0)
...
typedef struct gbHash_Table_Entry {
	u64 key;
	isize value;
	isize next;
} gbHash_Table_Entry;
...
#define GB_HASH_TABLE_HEADER             \
	...
	gbArray(gbHash_Table_Entry) entries  \

typedef struct gbHash_Table_Header {
...
typedef gbHash_Table(void) gbHash_Table_Void; // Generic Type
...
#define gb_hash_table_header(h) (cast(gbHash_Table_Header *)(h))
...
gb_inline void gb_mutex_init(gbMutex *m) { m->handle = CreateMutex(0, false, 0); }
gb_inline void gb_mutex_destroy(gbMutex *m) { CloseHandle(m->handle); }
gb_inline void gb_mutex_lock(gbMutex *m) { WaitForSingleObject(m->handle, INFINITE); }
gb_inline b32  gb_mutex_try_lock(gbMutex *m) { return WaitForSingleObject(m->handle, 0) == WAIT_OBJECT_0; }
gb_inline void gb_mutex_unlock(gbMutex *m) { ReleaseMutex(m->handle); }
...
{
	s->handle = CreateSemaphore(NULL, 0, I32_MAX, NULL);
	GB_ASSERT_MSG(s->handle != NULL, "CreateSemaphore: GetLastError = %d", GetLastError());
}

gb_inline void
gb_semaphore_destroy(gbSemaphore *s)
{
	BOOL err = CloseHandle(s->handle);
...
{
	BOOL err = ReleaseSemaphore(s->handle, count, NULL);
...
#if defined(GB_SYSTEM_WINDOWS)
	gb_internal DWORD WINAPI gbpriv_thread_proc(void *arg) { gbpriv_thread_run(cast(gbThread *)arg); return 0; }
#else
	gb_internal void *gbpriv_thread_proc(void *arg) { gbpriv_thread_run(cast(gbThread *)arg); return NULL; }
#endif

gb_inline void gb_thread_start(gbThread *t, gbThread_Proc *proc, void *data) { gb_thread_start_with_stack(t, proc, data, 0); }

gb_inline void
gb_thread_start_with_stack(gbThread *t, gbThread_Proc *proc, void *data, isize stack_size)
{
	...
	t->handle = CreateThread(NULL, stack_size, gbpriv_thread_proc, t, 0, NULL);
	GB_ASSERT_MSG(t->handle != NULL, "CreateThread: GetLastError = %d", GetLastError());
...
{
	if (str) {
		gbString_Header *header = GB_STRING_HEADER(str);
...
gb_string_trim(gbString str, char const *cut_set)
{
	...
	while (start_pos <= end && gb_strchr(cut_set, *start_pos))
		start_pos++;
	while (end_pos > start_pos && gb_strchr(cut_set, *end_pos))
		end_pos--;
...
char16 *
gb_from_utf8_to_utf16(char16 *buffer, char *s, isize len)
{
	u8 *str = cast(u8 *)s;
	u32 c;
	isize i = 0;
...
char *
gb_from_utf16_to_utf8(char *buffer, char16 *str, isize len)
{
	isize i = 0;
	len--;
	while (*str) {
